 package com.best.javaSdk.tmsGpsQuery.response;
 
 import com.best.javaSdk.tmsGpsQuery.response.DestConcernedGpsInfoBean;
 import com.best.javaSdk.tmsGpsQuery.response.HistoryGpsInfos;
 import com.best.javaSdk.tmsGpsQuery.response.SourceConcernedGpsInfoBean;
 
 
 public class TraceOmGpsList
 {
   private DestConcernedGpsInfoBean destConcernedGpsInfoBean;
   private HistoryGpsInfos historyGpsInfos;
   private SourceConcernedGpsInfoBean sourceConcernedGpsInfoBean;
   
   public DestConcernedGpsInfoBean getDestConcernedGpsInfoBean() {
/* 15 */     return this.destConcernedGpsInfoBean;
   }
 
   
   public void setDestConcernedGpsInfoBean(DestConcernedGpsInfoBean value) {
/* 20 */     this.destConcernedGpsInfoBean = value;
   }
 
   
   public HistoryGpsInfos getHistoryGpsInfos() {
/* 25 */     return this.historyGpsInfos;
   }
 
   
   public void setHistoryGpsInfos(HistoryGpsInfos value) {
/* 30 */     this.historyGpsInfos = value;
   }
 
   
   public SourceConcernedGpsInfoBean getSourceConcernedGpsInfoBean() {
/* 35 */     return this.sourceConcernedGpsInfoBean;
   }
 
   
   public void setSourceConcernedGpsInfoBean(SourceConcernedGpsInfoBean value) {
/* 40 */     this.sourceConcernedGpsInfoBean = value;
   }
 }


/* Location:              D:\wjw\Workspace\jiahe\wms-api.jar!\BOOT-INF\classes\com\best\javaSdk\tmsGpsQuery\response\TraceOmGpsList.class
 * Java compiler version: 8 (52.0)
 * JD-Core Version:       1.1.3
 */